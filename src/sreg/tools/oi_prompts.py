...
def build_oi_tools_section(artifact_catalog: list[dict[str, Any]]) -> str:
    ...
    return (
        ...
        "  - `oi` (instrumented helpers, see below)\n\n"
        "You can also import: `statsmodels`, `linearmodels` (for IV/FE/panel), "
        "`sklearn`, `scipy`. **NOT available**: seaborn, matplotlib.\n\n"
        "For common analyses, prefer the instrumented helpers:\n"
        "  oi.corr(df, cols=[\"A\", \"Y\"])          # correlation matrix\n"
        "  oi.regress(df, y=\"Y\", x=[\"A\", \"C\"])  # OLS regression\n"
        "  oi.stratify(df, by=\"Z\", value=\"Y\")   # stratified means\n"
        "  oi.test_independence(df, x=\"A\", y=\"Y\", z=\"C\")"
        "  # conditional independence\n"
        "  oi.groupby_mean(df, \"group\", \"value\") # grouped means\n"
        "  save_artifact(result_df, label=\"filtered\")  # save derived data\n"
        "Raw pandas/numpy operations are also fully available.\n\n"
        ...
        "ones.\n\n"
        "Each claim MUST include these fields:\n"
        "- claim_id: unique identifier (e.g. \"c1\", \"c2\")\n"
        "- claim_text: what you found, in natural language (15-800 chars)\n"
        "- focus_variables: which variables are involved (1-8)\n"
        "- confidence: how confident you are (0.0-1.0)\n"
        "- evidence_basis: [{artifact_id, rationale}] (1-5 refs)\n"
        "- relation_type: type of finding. One of:\n"
        "    causal_effect -- X causes/affects Y\n"
        "    mediation -- X affects Y through M (requires mediator field)\n"
        "    heterogeneity -- effect of X on Y varies by Z (requires modifier)\n"
        "    observational_association -- X and Y are correlated\n"
        "    confounding -- C creates spurious X-Y link (requires confounder)\n"
        "- treatment: the main cause/predictor variable (exact column name)\n"
        "- outcome: the main outcome variable (exact column name)\n"
        "- direction: positive, negative, or near_zero\n\n"
        "Optional fields for specific relation types:\n"
        "- mediator: for mediation, the intermediate variable (X->M->Y)\n"
        "- modifier: for heterogeneity, what modulates the effect\n"
        "- confounder: for confounding, what creates the spurious link\n"
        "- deliverable_index: which deliverable this addresses (0-indexed)\n\n"
        "IMPORTANT: \"No significant effect\" is a valid finding. Use "
        "direction=\"near_zero\" when you find no meaningful relationship.\n\n"
...
def build_oi_strategy_section() -> str:
    ...
    return (
        ...
        "- Use relation_type to distinguish: causal_effect for causal claims, "
        "observational_association for correlations.\n"
        "- Causal claims require adjustment for confounders or quasi-experimental "
        "evidence. A regression coefficient alone is not causal.\n"
        "- When in doubt about causation, use relation_type=\"observational_association\" "
        "with the correct treatment and outcome. You can still describe the "
        "relationship accurately in claim_text."
    )
