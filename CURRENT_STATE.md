...
- 6 hand-crafted SCMWorlds:
  - ecosystem (interaction), treatment (mediation+confounding), education (confounding+variance)
  - treatment_simpson (Simpson's paradox: crude r=-0.64, ATE=+0.4)
  - productivity (suppressor: crude r~0, partial r~0.72, ATE=+0.5)
  - screen_time (confounding reversal: crude r~+0.56, causal effect negative)
- 22 tests validating salience diversity + driver E2E + data-indexed properties

**Sub-question scoring (Session 9-10):**
- SubQuestionIntent model: pattern + roles + ask + tier
- Resolution pipeline: resolve_subquestion -> lower -> verify against SCM
- Matching: claim vs SQ via canonical signature + subsumption table
- Orchestrator generates SQs via design_case (validate + repair loop)
- E2E validated: 5 SQs generated by gpt-5.4, repair loop works
- `oi_subquestions.py`: 1049 lines, ~45 tests

**Investigation gap (Session 10-11):**
- No-data baseline probe: `scripts/oi_nodata_baseline.py`
- 4 data-indexed worlds (force investigation): ecosystem +0.570, productivity
  +0.488, screen_time +0.350, treatment_simpson +0.132
- 2 non-data-indexed worlds (priors sufficient): treatment -0.093, education 0.000
- Analysis: `research/notes/oi_investigation_gap.md`
