...
- Mini benchmark (`scripts/mini_benchmark.py`): 3 SRCs reales via orchestrator, solo infer_target.
- **S.2 diagnostic pipeline** (`scripts/diagnostic_batch.py`): N SRCs via orchestrator real, agent
  en CADA task (multi-tipo). Metricas por eval type, failure modes, report diagnostico.
  Primer diagnostico: 3 SRCs, 11 tasks, 7/9 tipos ejercitados, 91% submit, 0 format errors.
  Hallazgo: choice types tienden a ser triviales o wrong. Distribution types funcionan bien.
- `quality.py` (QualitySuite v2) solo cubre metricas estructurales del motor formal (3/9 eval
  types, sin LLM). Pendiente: BenchmarkRunner consolidado, metrica prior_delta.
