"""Full per-target dump for Suite 2 — v3 (post-compiler-fix).

v3 measures the effect of the compiler changes in the `compiler-fix`
worktree (epic #36):
  - Task #6 schema split: deliberate_abstention vs fallback abstention.
  - Task #1 H0 port: Flow A now composes abstention exemplars + checks
    `is_explicit_abstention` before JSON parse.
  - Task #2: QueryArm validator rejects `CONDITION|BASELINE + values`;
    GRAMMAR_REF rewritten with explicit kind-selection table.
  - Task #3: TARGETED_RECIPE_EXEMPLARS (Recipes C-F + escape hatches)
    wired into both flows.

Outputs:
  - research/synthesis/compiler_baseline_full_dump_v3.json
  - research/synthesis/compiler_baseline_failures_v3.json
  - research/synthesis/compiler_baseline_full_dump_v3.jsonl (streaming log)
...
OUT_FULL = Path("research/synthesis/compiler_baseline_full_dump_v3.json")
OUT_FAILS = Path("research/synthesis/compiler_baseline_failures_v3.json")
