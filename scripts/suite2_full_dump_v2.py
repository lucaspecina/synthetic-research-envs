"""Full per-target dump for Suite 2 — all 55 gold targets with bucket category.

Closes I-027 item 4 (reproducibility gap). Produces two artifacts:

  - research/synthesis/compiler_baseline_full_dump_v2.json
      Source of truth. 55 entries, each with compiler output, gold specs,
      stage verdicts, atom verdicts, and bucket category.
  - research/synthesis/compiler_baseline_failures_v2.json
      Derived subset: entries whose category is not full_pass / adjust_swap.

The v1 artifact `compiler_baseline_failures.json` is NOT overwritten — it
remains the canonical snapshot of the 2026-04-14 baseline for traceability
with `suite2_compiler_baseline.md`.
...
OUT_FULL = Path("research/synthesis/compiler_baseline_full_dump_v2.json")
OUT_FAILS = Path("research/synthesis/compiler_baseline_failures_v2.json")
    ...
    for i, gt in enumerate(ordered_targets, start=1):
        ...
        if call_err is not None or cout is None:
            ...
            entry = {
                ...
                "compiler_abstain_reason": getattr(cout, "abstain_reason", None),
            }
